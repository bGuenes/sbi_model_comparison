...
train_data = torch.cat((train_x, train_y), 1)[:10000]
...
plt.plot(epoch, np.array(ModelTransfuser.train_loss)/1000, label='Train Loss')
plt.plot(epoch, np.array(ModelTransfuser.val_loss)/val_data.shape[0], label='Val Loss')
plt.legend()
plt.xlabel('Epoch')
plt.ylabel('Loss')

plt.savefig('plots/ModelTransfuser_train_loss_e100_s101_lr_1000data.png')
